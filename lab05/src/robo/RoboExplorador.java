...
public class RoboExplorador extends AgenteInteligente {
    ...
    public RoboExplorador(int X, int Y, int Z, String id, Sensor sensor, EstadoRobo estado, Missao missao) {
    ...
    public void executarMissao(Ambiente a) throws Exception {
        if (getEstado() == EstadoRobo.LIGADO) {
            if (this.missao instanceof MissaoExplorar) {
                if (temMissao()) {
                    System.out.println("Executando missao para explorar...");
                    try { 
